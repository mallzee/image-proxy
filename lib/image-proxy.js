...
var express = require('express')
  ...
  , delay = parseInt(process.env.DELAY) || 50000
  , mimeTypes = [
    'image/gif',
    'image/jpeg',
    'image/png',
...
module.exports = function () {
  ...
  app.get('/:url/:width/:height', function (req, res, next) {
    ...
    if (isNaN(parseInt(width))) {
      console.log('Expected width to be an integer', width, req.params.url);
      return res.status(404).send('Expected width to be an integer');
    }
    if (parseInt(width) > MAX_WIDTH) {
      console.log('Expected width to be less than integer', MAX_WIDTH, req.params.url);
      return res.status(404).send('Expected width to be less than or equal to ' + MAX_WIDTH);
    }
    if (isNaN(parseInt(height))) {
      console.log('Expected height to be an integer', height, req.params.url);
      return res.status(404).send('Expected height to be an integer');
    }
    if (parseInt(height) > MAX_HEIGHT) {
      console.log('Expected height to be less than integer', MAX_HEIGHT, req.params.url);
